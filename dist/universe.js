...
(()=>{
 ...
 canvas.dataset.renderer='webgl';renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<700?1.15:1.35));renderer.setClearColor(0,0);
 const scene=new T.Scene(),camera=new T.PerspectiveCamera(40,1,.1,100);camera.position.z=8.5;
 const count=hero?(innerWidth<700?8000:14000):10000,geometry=new T.BufferGeometry(),positions=new Float32Array(count*3),seeds=new Float32Array(count*3);let seed=91;const rand=()=>{seed=seed*16807%2147483647;return(seed-1)/2147483646};
 ...
 const frame=now=>{raf=requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.04);last=now;if(!visible||document.hidden||document.documentElement.dataset.transit)return;if(paused()&&!dirty)return;if(!paused()){time+=dt;px+=(tx-px)*.035;py+=(ty-py)*.035;phase+=(targetPhase-phase)*Math.min(1,dt*2.5)}else phase=targetPhase;
