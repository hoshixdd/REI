...
(function(root){
 ...
 function planImport(existing,incoming){const seenD=new Set(existing.map(p=>doi(p.doi)).filter(Boolean)),seenT=new Set(existing.map(p=>titleKey(p.title)));return incoming.map(p=>{let reason=!p.title?'Missing title':(p.doi&&seenD.has(p.doi))||seenT.has(titleKey(p.title))?'Duplicate DOI or title':'';if(!reason){seenT.add(titleKey(p.title));if(p.doi)seenD.add(p.doi)}return {paper:p,reason}})}
  function normalize(p){
   ...
   for(const k of ['claims','design','decisions','snapshots','searches','comparisons','reviews','codes','coding','reviewActions','reviewResolved'])if(!Array.isArray(w[k]))w[k]=[];
   for(const d of w.design)for(const k of ['population','sample','ethics','feasibility','output'])if(typeof d[k]!=='string')d[k]='';
   for(const c of w.codes)for(const k of ['label','definition','color'])if(typeof c[k]!=='string')c[k]=k==='color'?'#65dbff':'';
   for(const c of w.coding)for(const k of ['evidenceId','codeId','note'])if(typeof c[k]!=='string')c[k]='';
   return w;
  }
 function validateWorkbench(p){
  if(p.workbench===undefined)return;
   ...
   for(const k of ['codes','coding','reviewActions','reviewResolved'])if(w[k]===undefined)w[k]=[];
  const papers=new Set(p.papers.map(x=>x.id)),evidence=new Set(p.evidence.map(x=>x.id));
   for(const [kind,fields] of Object.entries({claims:['id','text','status'],design:['id','objective','measure','collection','analysis','rationale'],decisions:['id','text','date'],snapshots:['id','date','proposal','question'],searches:['id','query','database','date','notes'],comparisons:['id','left','right','explanation'],reviews:['id','prompt','answer'],codes:['id','label','definition','color'],coding:['id','evidenceId','codeId','note'],reviewActions:['id','text','area','status','date']})){
   if(!Array.isArray(w[kind])||w[kind].length>2000)throw Error('Invalid workbench records.');const ids=new Set();for(const r of w[kind]){if(!r||fields.some(k=>typeof r[k]!=='string')||ids.has(r.id))throw Error('Invalid '+kind+' record.');ids.add(r.id);
    if(kind==='claims'){if(!['Tentative','Working','Reviewed'].includes(r.status)||!Array.isArray(r.links))throw Error('Invalid claim.');for(const l of r.links)if(!evidence.has(l.evidenceId)||!['supports','challenges','mixed','uncertain'].includes(l.relation))throw Error('Broken evidence link.')}
     ...
     if(kind==='codes'&&!['#65dbff','#ae9bff','#ffab91','#a7e06f'].includes(r.color))throw Error('Invalid synthesis theme.');
     if(kind==='coding'&&(!evidence.has(r.evidenceId)||!new Set(w.codes.map(c=>c.id)).has(r.codeId)))throw Error('Broken coding record.');
     if(kind==='reviewActions'&&!['Open','Done'].includes(r.status))throw Error('Invalid review action.');
    }
   }
   if(w.reviewResolved.some(x=>typeof x!=='string'))throw Error('Invalid resolved review action.');
  ...
  function checks(p){const w=normalize(p);const findings=[];if(!p.question.trim())findings.push('Record a research question.');if(!w.design.length)findings.push('Add at least one objective and methods plan.');for(const [i,d] of w.design.entries())for(const [key,label] of Object.entries({objective:'objective',measure:'measure or construct',collection:'collection method',analysis:'analysis plan',rationale:'alignment rationale',population:'population or setting',sample:'sample or selection',ethics:'ethics and privacy plan',feasibility:'feasibility plan',output:'expected output or result'}))if(!d[key].trim())findings.push('Objective '+(i+1)+': add '+label+'.');const linked=new Set();for(const [i,c] of w.claims.entries()){if(!c.links.length)findings.push('Claim '+(i+1)+' has no linked evidence.');for(const l of c.links)linked.add(l.evidenceId);if(c.links.some(l=>l.relation==='challenges'))findings.push('Claim '+(i+1)+' has challenging evidence to address.');if(c.links.length===1)findings.push('Claim '+(i+1)+' relies on one evidence note; compare another source if possible.')}for(const [i,n] of p.evidence.entries())if(!linked.has(n.id))findings.push('Evidence E'+(i+1)+' is not connected to a claim yet.');for(const [i,c] of w.comparisons.entries())if(!c.explanation.trim())findings.push('Comparison '+(i+1)+' needs an explanation before it can inform synthesis.');if(p.proposal.trim()&&!w.claims.length)findings.push('Proposal text exists, but no claims are recorded in the evidence trail.');if(w.impact)findings.push('Your question changed: review the objectives, evidence, and draft.');return findings}
  function sourceAudit(p){const w=normalize(p),issues=[],val=x=>String(x??'').trim();for(const [i,s] of p.papers.entries()){if(!val(s.authors))issues.push('Source S'+(i+1)+' is missing author information.');if(!val(s.year))issues.push('Source S'+(i+1)+' is missing a publication year.');if(!val(s.venue))issues.push('Source S'+(i+1)+' is missing a publication or venue.');if(!val(s.doi)&&!val(s.url))issues.push('Source S'+(i+1)+' has no DOI or source URL.')}for(const [i,n] of p.evidence.entries()){if(val(n.quote)&&!val(n.locator))issues.push('Evidence E'+(i+1)+' has an exact passage but no page or section locator.');if(!val(n.findings))issues.push('Evidence E'+(i+1)+' is missing a recorded finding.');if(!val(n.limitations))issues.push('Evidence E'+(i+1)+' is missing a limitation or boundary.');if(!val(n.confidence))issues.push('Evidence E'+(i+1)+' is missing interpretation confidence.')}for(const [i,c] of w.claims.entries())if(c.links.some(l=>{const n=p.evidence.find(n=>n.id===l.evidenceId);return !val(n?.quote)&&!val(n?.locator)}))issues.push('Claim '+(i+1)+' links evidence without a passage or location.');return issues}
   function alignment(p){const w=normalize(p);return w.design.map((d,i)=>({index:i+1,objective:d.objective,gaps:['measure','population','sample','collection','analysis','ethics','feasibility','output','rationale'].filter(k=>!d[k].trim()),ready:['objective','measure','population','sample','collection','analysis','ethics','feasibility','output','rationale'].every(k=>d[k].trim())}))}
  function readiness(p){const w=normalize(p),linked=new Set(w.claims.flatMap(c=>c.links.map(l=>l.evidenceId)));return [{id:'question',label:'Question clarity',ready:Boolean(p.question.trim())},{id:'sources',label:'Source coverage',ready:p.papers.length>0},{id:'traceability',label:'Evidence traceability',ready:p.evidence.length>0&&w.claims.length>0&&p.evidence.every(n=>linked.has(n.id))},{id:'methods',label:'Method alignment',ready:w.design.length>0&&w.design.every(d=>['objective','measure','population','sample','collection','analysis','ethics','feasibility','output','rationale'].every(k=>d[k]))},{id:'ethics',label:'Ethics planning',ready:w.design.some(d=>/ethic|consent|privacy|risk/i.test((d.ethics||'')+' '+(d.rationale||'')))},{id:'feasibility',label:'Feasibility',ready:w.design.some(d=>/access|time|resource|feasib|permission/i.test((d.feasibility||'')+' '+(d.rationale||'')))},{id:'proposal',label:'Proposal coherence',ready:Boolean(p.proposal.trim()&&w.claims.length)},{id:'review',label:'Review readiness',ready:w.reviews.length>0&&w.decisions.length>0}]}
 function bibliography(p){return p.papers.map((s,i)=>`[${i+1}] ${s.authors||'Author not recorded'} (${s.year||'n.d.'}). ${s.title}. ${s.venue||''} ${s.doi?'https://doi.org/'+s.doi:s.url||''}`).join('\n\n')}
  const api={csvRows,bibRecords,parseImport,planImport,normalize,validateWorkbench,checks,sourceAudit,alignment,readiness,bibliography,doi};if(typeof module!=='undefined')module.exports=api;else root.REICore=api;
