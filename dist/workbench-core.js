...
(function(root){
 ...
 function validateWorkbench(p){
  ...
  for(const n of p.evidence){if(n.quote!==undefined&&typeof n.quote!=='string')throw Error('Invalid passage.');if(n.page!==undefined&&(!Number.isInteger(n.page)||n.page<1))throw Error('Invalid page.')}
 ...
 function checks(p){const w=normalize(p);const findings=[];if(!p.question.trim())findings.push('Record a research question.');if(!w.design.length)findings.push('Add at least one objective and methods plan.');for(const [i,d] of w.design.entries())for(const [key,label] of Object.entries({objective:'objective',measure:'measure or construct',collection:'collection method',analysis:'analysis plan',rationale:'alignment rationale'}))if(!d[key].trim())findings.push('Objective '+(i+1)+': add '+label+'.');for(const [i,c] of w.claims.entries()){if(!c.links.length)findings.push('Claim '+(i+1)+' has no linked evidence.');if(c.links.some(l=>l.relation==='challenges'))findings.push('Claim '+(i+1)+' has challenging evidence to address.')}if(w.impact)findings.push('Your question changed: review the objectives, evidence, and draft.');return findings}
 ...
 const api={csvRows,bibRecords,parseImport,planImport,normalize,validateWorkbench,checks,bibliography,doi};if(typeof module!=='undefined')module.exports=api;else root.REICore=api;
